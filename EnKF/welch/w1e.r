...
w1 <- function(Q,make.plots=TRUE,nmemb=20)  {
  # scalar, fill
  ...
  P0e <- mvrnorm(n=nmemb,mu=x0,Sigma=sqrt(P0))
  r <- 0.1 # measurement noise (std dev)
  R <- r^2 #  measurement noise (variance)
  n <- 100
  
  y <- rep(NA,n)
  dy <- matrix(NA,nrow=n,ncol=nmemb)
  x <- rep(NA,n)
  dx <-  matrix(NA,nrow=n,ncol=nmemb)
  state <- list()
  state$x.prior <- rep(NA,n)
  state$dx.prior <- matrix(NA,nrow=n,ncol=nmemb)
  state$x.post <- rep(NA,n)
  state$dx.post <- matrix(NA,nrow=n,ncol=nmemb)
...
#    d <- L[i]
    ...
    this.dx <- matrix(dx[i,],ncol=1,nrow=nmemb)
    this.H <- matrix(H,1,1)
    this.d <- matrix(d,1,1)
    this.Pd <- matrix(Pd,1,1)
    xtrue <- matrix(L[i],1,1)
    # forward model
    dy <- (as.numeric(this.x)+this.dx) %*%this.H
    
    post <- enkf.measurement.update(x=this.x,dx=t(this.dx),
                                    d=this.d,Pd=this.Pd,
                                    y=y[i],dy=t(dy),
                                    xtrue=xtrue)
    x[i] <- post$x[1,1]
